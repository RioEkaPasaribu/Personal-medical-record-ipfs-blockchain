...
exports.postHealthRecord = catchAsync(async (req, res, next) => {
    ...
    // // // Upload File - Enkripsi AES
    // const encryptionResult = await aesUtil.encryptFile(req.body.patientAddress, currentUser.aesKey, req.file)

    // // Ambil hasil enkripsi - coba berbagai kemungkinan format
    // let encryptedData;
    // if (typeof encryptionResult === 'string') {
    //     encryptedData = encryptionResult;
    // } else if (encryptionResult && encryptionResult.encryptedData) {
    //     encryptedData = encryptionResult.encryptedData;
    // } else if (encryptionResult && encryptionResult.data) {
    //     encryptedData = encryptionResult.data;
    // } else {
    //     // Fallback: baca file hasil enkripsi
    //     const pathFile = `./public/files/uploads/${req.body.patientAddress}-data.txt`
    //     if (fs.existsSync(pathFile)) {
    //         encryptedData = fs.readFileSync(pathFile, 'utf8');
    //     } else {
    //         return next(new AppError('Gagal melakukan enkripsi file.', 400))
    //     }
    // }

    // // Validasi bahwa encryptedData tidak kosong
    // if (!encryptedData || encryptedData.trim() === '') {
    //     return next(new AppError('Hasil enkripsi tidak valid.', 400))
    // }
    ...
    // Data ABI untuk Private Transaction - Upload langsung ke blockchain
    const dataPMR = {
        senderAddress: req.authData.address,
        patientAddress: req.body.patientAddress,
        newRecordId: uuidv4(),
        description: req.body.description,
        // cid: encryptedData, // Menggunakan hasil enkripsi AES langsung
    ...
    // console.log("Data PMR:", dataPMR);
    // console.log("Object values:", Object.values(dataPMR));
    ...
    console.log(`File asli: ${originalSize} bytes`);
    const result = await web3Util.privateSendTransactionWithGasInfo(privateTX.encodeABI());
    nodeBest.done();
